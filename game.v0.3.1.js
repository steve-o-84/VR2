...
if(navigator.xr?.isSessionSupported){navigator.xr.isSessionSupported('immersive-vr').then(supported=>window.dispatchEvent(new CustomEvent('velvethour-vr-support',{detail:{supported}}))).catch(()=>window.dispatchEvent(new CustomEvent('velvethour-vr-support',{detail:{supported:false}})));}else{window.dispatchEvent(new CustomEvent('velvethour-vr-support',{detail:{supported:false}}));}
...
async function start(desktop=true){
  if(!desktop){
    const supported=!!navigator.xr && await navigator.xr.isSessionSupported('immersive-vr').catch(()=>false);
    if(!supported){document.getElementById('bootStatus').className='error';document.getElementById('bootStatus').textContent='WebXR ist in diesem Browser/Gerät nicht verfügbar. Öffne die Seite im Meta Quest Browser.';return;}
  }
  state.started=true;document.getElementById('intro').classList.add('hidden');document.querySelectorAll('.game-ui').forEach(e=>e.classList.remove('hidden'));
  if(!desktop){vrButton.style.display='block';toast('Unten rechts ENTER VR auswählen.')}
  renderUI();
}
...
window.dispatchEvent(new CustomEvent('velvethour-ready'));
